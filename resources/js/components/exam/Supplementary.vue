        ...
        <div style="margin-top: 20px">
          <v-simple-table fixed-header>
            <template v-slot:default>
              ...
              <tbody>
              <tr v-if="supplementaryExamList.length === 0">
                <td colspan="9" class="text-center">Tidak ada data remedial</td>
              </tr>
              <tr v-else v-for="(item, index) in supplementaryExamList" :key="index">
                <td>{{ index + 1 }}</td>
                  ...
                  <center>
                    <div>
                      <span v-if="today < convertFormatDate(item.start_date)">
                        <v-chip class="ma-2" color="red" text-color="white" x-small>Ujian susulan belum dimulai</v-chip>
                      </span>
                      <span v-else-if="today > convertFormatDate(item.end_date)">
                        <span v-if="item.assign_student.exam.show_value === 2">
                          <v-chip class="ma-2" color="green" text-color="white"
                                  x-small>Ujian susulan sudah selesai</v-chip>
                        </span>
                        <span v-else>
                          <v-btn @click="getScore(item.assign_student.exam.id)" color="success" title="Hasil Ujian" fab
                                 small dark>
                      ...
                      <span v-else>
                        <span v-if="item.status === 1">
                          ...
                          <span v-else>
                            <v-btn @click="getScore(item.assign_student.exam.id, item.id)" color="success"
                                   title="Hasil Ujian" fab small dark>
                              <v-icon>mdi-chart-bar</v-icon>
                            </v-btn>
                          </span>
                        </span>
...
export default {
  ...
  methods: {
        ...
        callback: confirm => {
          if (confirm) {
            axios.post('/exam/start', {
              ...
              localStorage.setItem('type', 'supplementary');
    ...
    getExam() {
      ...
      axios.get('/exam/supplementary', {
        params: {
          page: this.pagination.current,
          class_id: id
        },
      }).then(resp => {
        this.supplementaryExamList = resp.data.data.data;
    ...
    getScore(id, supplementaryId) {
      this.dialog = true;
      axios.get('/exam/score/student/supplementary', {
        params: {
          exam_id: id,
